...
export default function AdminVideos() {
    ...
    const handleSaveEdit = async (id) => {
        if (!editName.trim()) {
            setMessage({
                text: "Video name cannot be empty",
                type: "error",
            });
            return;
        }
        ...
        try {
            ...
            setMessage({
                text: `Failed to update video details: ${error.message}`,
                type: "error",
            });
        } finally {
            setIsLoading(false);
                        ...
                        {viewMode === "grid" ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                {videos.map((video) => (
                                    <div
                                        key={video.id}
                                        className={`bg-gray-50 rounded-lg overflow-hidden shadow hover:shadow-md transition-shadow ${video.archived ? "opacity-60" : ""
                                            }`}
                                    >
                                            ...
                                            {editingVideoId === video.id ? (
                                                <div className="space-y-2 mb-2">
                                                    <input
                                                        ...
                                                        className="w-full px-2 py-1 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-[#EE3224] text-black text-sm"
                            ...
                            <div className="divide-y">
                                {videos.map((video) => (
                                    <div
                                        key={video.id}
                                        className={`py-4 flex flex-col sm:flex-row ${video.archived ? "opacity-60" : ""
                                            }`}
                                    >
                                            ...
                                            {editingVideoId === video.id ? (
                                                <div className="space-y-2 mb-2">
                                                    <input
                                                        ...
                                                        value={editName}
                                                        onChange={(e) => setEditName(e.target.value)}
                                                        className="w-full px-2 py-1 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-[#EE3224] text-black"
                                                        placeholder="Video title"
                                                    />
                                                    <textarea
                                                        value={editDescription}
